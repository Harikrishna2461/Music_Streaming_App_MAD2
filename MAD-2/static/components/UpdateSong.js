export default {
    ...
    mounted() {
        // Fetch songs and albums from the backend when the component is mounted
        this.fetchSongs();
        this.fetchAlbums();
    },
    watch: {
        ...
        async updateSong() {
            try {
                ...
                if (parseInt(this.selectedSong.creator_id) !== parseInt(currentUserId)) {
                    console.error('You are not authorized to update this song.');
                    alert('You are not authorized to update this song.')
                    return;
                }
